...
Web app to record archery training sessions. User marks arrows on an SVG target (click + zoom), app auto-calculates scores and tracks running totals per session.
...
- SQLite via Prisma ORM (`DATABASE_URL=file:./dev.db`)
...
### Flint Round
- 7 ends × 4 arrows = 28 arrows total
- Scoring: X (5pts), 5, 4, 3, M (0pts)
- Distances and target auto-selected per end:
...
| 4   | 20yd     | 4-spot Flint |
| 5   | 10yd     | 4-spot Flint |
| 6   | Walk-up (30/25/20/15yd per arrow) | 1-spot Flint |

## Target Variants

All targets use `viewBox="0 0 200 200"`. Arrow (x,y) stored in SVG coords. Arrow dot radius = 3 SVG units.
...
4. Walk-up end 6: distance label updates per arrow (30yd → 25yd → 20yd → 15yd), 1-spot target
...
- [x] Phase 1: Domain layer (`src/lib/domain/`) + unit tests (65/65 passing)
- [x] Phase 2: Prisma schema + migration (Prisma 7 + better-sqlite3 adapter)
- [x] Phase 3: API routes (sessions, ends, arrows with double-hit logic)
- [x] Phase 4: Client API helpers (`src/lib/api/client.ts`)
- [x] Phase 5: UI components (target SVG, zoom overlay, scoring table, session cards)
- [x] Phase 6: Pages (home, marking screen, summary)
